...
if (isset($_GET['print']) && isset($_GET['id'])) {
    ...
    if (!empty($report['report_filter'])) {
        ...
        if (is_array($filter_data)) {
            if (isset($filter_data['powder']))
                $powder_filter = $filter_data['powder'];
            elseif (isset($filter_data['powder_type']))
                $powder_filter = $filter_data['powder_type'];

            if (isset($filter_data['surface']))
                $surface_filter = $filter_data['surface'];
            elseif (isset($filter_data['surface_type']))
                $surface_filter = $filter_data['surface_type'];

            if (isset($filter_data['student_id']))
                $student_filter = $filter_data['student_id'];
            if (isset($filter_data['from']))
                $from_filter = $filter_data['from'];
            if (isset($filter_data['to']))
                $to_filter = $filter_data['to'];
        }
    }
        ...
        <div class="metadata-grid">
            ...
            <div class="metadata-item">
                ...
                <span class="metadata-value"
                    style="text-transform: capitalize;"><?= $powder_filter ? htmlspecialchars($powder_filter) : 'All Powders' ?></span>
            </div>
            <div class="metadata-item">
                <span class="metadata-label">Surface Material Filter:</span>
                <span class="metadata-value"
                    style="text-transform: capitalize;"><?= $surface_filter ? htmlspecialchars($surface_filter) : 'All Surfaces' ?></span>
            </div>
        </div>
        ...
        <table class="results-table">
            ...
            <tbody>
                <?php if (empty($matching_trials)): ?>
                    <tr>
                        <td colspan="6" style="text-align: center; color: #666;">No approved records matching report criteria.
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($matching_trials as $trial): ?>
                        <tr>
                            <td style="font-weight: 700; color: #1b4332;">
                                <?= htmlspecialchars($trial['trial_id'] ?: 'TR-' . str_pad($trial['id'], 4, '0', STR_PAD_LEFT)) ?>
                            </td>
...
<body>
    ...
    <div class="student-wrapper">
        ...
        <div id="sidebarOverlay"
            style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:999;transition:opacity .3s;"
                                    ...
                                    <?php foreach ($reports_list as $row): ?>
                                        <tr>
                                            ...
                                            <td style="text-align: right;">
                                                <a href="approved_reports.php?print=1&id=<?= $row['id'] ?>" target="_blank"
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    <?php foreach ($all_approved_trials as $row): ?>
                                        <tr>
                                            <td style="font-weight:700; color:var(--dark-green);">
                                                <?= htmlspecialchars($row['trial_id'] ?: 'TR-' . str_pad($row['id'], 4, '0', STR_PAD_LEFT)) ?>
                                            </td>
                                            <td><?= htmlspecialchars($row['student_name']) ?></td>
                                            <td style="text-transform:capitalize;">
                                                <?= $row['powder_type'] === 'eggshell' ? 'Eggshell-Based' : 'Commercial' ?></td>
                                            <td style="text-transform:capitalize;"><?= htmlspecialchars($row['surface_type']) ?>
                                            </td>
                                            ...
                                            <td style="text-align: right;">
                                                <button class="btn btn-secondary btn-sm"
                                                    onclick="openDetailsModal(<?= htmlspecialchars(json_encode($row)) ?>)">View
                                                    Details</button>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
    ...
    <div class="detail-overlay" id="detailsModal">
        <div class="detail-modal">
            ...
            <div class="detail-modal-body">
                ...
                <div class="detail-row"><span class="detail-label">Student Submitter</span><span class="detail-value"
                        id="modalStudent"></span></div>
                <div class="detail-row"><span class="detail-label">Powder Type Used</span><span class="detail-value"
                        id="modalPowder" style="text-transform: capitalize; font-weight: 600;"></span></div>
                <div class="detail-row"><span class="detail-label">Surface Material Type</span><span
                        class="detail-value" id="modalSurface"
                        style="text-transform: capitalize; font-weight: 600;"></span></div>
                <div class="detail-row"><span class="detail-label">Image Label</span><span class="detail-value"
                        id="modalLabel"></span></div>
                <div class="detail-row"><span class="detail-label">Notes from Submission</span><span
                        class="detail-value" id="modalNotes"></span></div>
                <div class="detail-row"><span class="detail-label">Date Submitted</span><span class="detail-value"
                        id="modalDateSubmitted"></span></div>
                ...
                <div
                    ...
                    <div id="modalImageFallback"
                        style="padding:2rem; background:#f4f6f0; border-radius:6px; font-weight:600; color:var(--gray); display:none;">
                ...
                <div class="detail-row"
                    ...
                    <span class="detail-value" id="modalAccuracy"
                        style="font-weight: 800; color: var(--dark-green); font-size:1.1rem;">—</span>
                </div>

                <p class="section-divider">Validation Details</p>
                <div class="detail-row"><span class="detail-label">Validation Status</span><span
                        class="detail-value"><span class="badge-approved">Approved</span></span></div>
                <div class="detail-row"><span class="detail-label">Faculty Validator</span><span class="detail-value"
                        id="modalValidator" style="font-weight: 600;"></span></div>
                <div class="detail-row"><span class="detail-label">Validation Date</span><span class="detail-value"
                        id="modalDateValidated"></span></div>
                <div class="detail-row"><span class="detail-label">Advisor Remarks</span><span class="detail-value"
                        id="modalRemarks" style="font-style: italic;"></span></div>
            </div>
        </div>
    </div>
