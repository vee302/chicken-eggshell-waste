            ...
            <div class="student-content">
                <div class="page-header-wrap">
                    <div class="page-title">
                        ...
                        <p>Welcome back, <?= htmlspecialchars($student_name) ?>. Here is a summary of your forensic
                            submissions.</p>
                    </div>
                ...
                $student_phone = '';
                try {
                    $u_stmt = $pdo->prepare("SELECT contact_number FROM users WHERE id = ?");
                    $u_stmt->execute([$student_id]);
                    $student_phone = $u_stmt->fetchColumn() ?: '';
                } catch (PDOException $e) {}
                ?>
                ...
                <script>
                function promptContactUpdate() {
                    const phone = prompt("Enter your Philippine Mobile Phone Number (e.g. 09171234567):");
                    if (!phone) return;
                    
                    const formData = new FormData();
                    formData.append("contact_number", phone);
                    
                    fetch("ajax_update_contact_number.php", {
                        method: "POST",
                        body: formData
                    })
                    .then(res => res.json())
                    ...
                    .catch(err => {
                        alert("An error occurred while updating mobile number.");
                    });
                }
                </script>
                <?php endif; ?>
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-value" id="val-avg"
                            style="font-size: <?= (strlen($avg_display) > 6) ? '1.25rem' : '2rem' ?>;">
                            <?= htmlspecialchars($avg_display) ?></div>
                                ...
                                <?php if (empty($recent)): ?>
                                    <tr>
                                        <td colspan="6" style="text-align:center;color:#6c757d;padding:2rem;">
                                            No submissions yet. Upload your first fingerprint image to begin evaluation. <a
                                                href="upload_fingerprint.php"
                                                style="color:var(--medium-green);font-weight:600;">Upload now →</a>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($recent as $row): ?>
                                        <tr data-trial-db-id="<?= $row['id'] ?>"
                                            onclick='openDetailModal(<?= htmlspecialchars(json_encode($row), ENT_QUOTES, "UTF-8") ?>)'>
                                            <td style="text-transform:capitalize;"><?= htmlspecialchars($row['powder_type']) ?>
                                            </td>
                                            ...
                                            <td style="text-align:right;">
                                                <button class="btn btn-secondary btn-sm view-details-btn"
                                                    onclick='event.stopPropagation(); openDetailModal(<?= htmlspecialchars(json_encode($row), ENT_QUOTES, "UTF-8") ?>)'
                                                    style="padding: 0.35rem 0.75rem; font-size: 0.75rem; border-radius: 6px;">View
                                                    Details</button>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
