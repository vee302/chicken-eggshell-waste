...
<head>
    ...
    <style>
        ...
        /* Clean Light Modal Styling for Detailed Quality Inspection & Side-by-Side Comparison */
        ...
        #comparisonOverlay .detail-modal-header {
            background: var(--dark-green, #1b4332) !important;
        ...
        /* Column Titles & Section Dividers */
        .column-title {
            font-size: 0.78rem;
            font-weight: 700;
            color: #6B8F71;
        ...
        .notes-text {
            ...
            min-height: 45px;
            border: 1px solid #e9ecef;
            border-left: 4px solid var(--medium-green, #2d6a4f);
        }
        ...
        .comp-btn-outline {
            ...
            color: var(--dark-green, #1b4332) !important;
        ...
        .comp-btn-primary {
            ...
            min-height: 46px;
            background: var(--dark-green, #1b4332) !important;
            border: 1px solid var(--dark-green, #1b4332) !important;
            color: #ffffff !important;
...
<body>
            ...
            <div class="student-content">
                ...
                <form id="filterForm" class="filter-bar no-print">
                    <div class="filter-item">
                        ...
                        <select name="status" id="filter-status">
                            ...
                            <option value="needs_revision" <?= $filter_status === 'needs_revision' ? 'selected' : '' ?>>
                    ...
                    <div class="filter-item">
                        ...
                        <select name="powder" id="filter-powder">
                            ...
                            <option value="eggshell" <?= $filter_powder === 'eggshell' ? 'selected' : '' ?>>Eggshell
                            </option>
                            <option value="commercial" <?= $filter_powder === 'commercial' ? 'selected' : '' ?>>Commercial
                            </option>
                        </select>
                    </div>
                    <div class="filter-item">
                        ...
                        <select name="surface" id="filter-surface">
                            ...
                            <?php foreach (['glass', 'plastic', 'metal', 'wood'] as $s): ?>
                                <option value="<?= $s ?>" <?= $filter_surface === $s ? 'selected' : '' ?>><?= ucfirst($s) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="filter-item" style="justify-content:flex-end;">
                        ...
                        <button type="button" class="btn btn-secondary btn-sm" id="btnClearFilters"
                            style="display: <?= ($filter_status || $filter_powder || $filter_surface) ? 'inline-block' : 'none' ?>;">Clear
                            Filters</button>
                    </div>
                </form>
                ...
                <div class="dashboard-card">
                    <div class="card-title-wrap">
                        ...
                        <span id="recordCount" style="font-size:.82rem;color:var(--gray);"><?= count($records) ?>
                            record<?= count($records) !== 1 ? 's' : '' ?></span>
                    </div>
                                    ...
                                    <tr class="no-data-row">
                                        <td colspan="11" style="text-align:center;color:#6c757d;padding:2.5rem;">
                                            ...
                                            <?php if (!$filter_status && !$filter_powder && !$filter_surface): ?>
                                                <a href="upload_fingerprint.php"
                                                    style="color:var(--medium-green);font-weight:600;">Upload your first
                                                    fingerprint image to begin evaluation →</a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($records as $i => $r): ?>
                                        <tr data-trial-db-id="<?= $r['id'] ?>"
                                                ...
                                                <?php if ($r['image_path']): ?>
                                                    <?php if ($r['image_exists']): ?>
                                                        <a href="../view_fingerprint.php?test_id=<?= $r['id'] ?>" target="_blank"
                                                            ...
                                                            <img src="../view_fingerprint.php?test_id=<?= $r['id'] ?>"
                                                                style="width:48px;height:48px;object-fit:cover;border-radius:8px;border:1px solid #e9ecef;"
                                                                alt="FP">
                                                        </a>
                                                    <?php else: ?>
                                                        <span style="font-size: 0.72rem; color: var(--danger); font-weight:600;">Image
                                                            not found</span>
                                                    <?php endif; ?>
                                                <?php else: ?>
                                                    <span style="font-size: 0.72rem; color: var(--gray); font-style:italic;">No
                                                        image</span>
                                                <?php endif; ?>
                                            </td>
                                                    ...
                                                    <div class="score-bar">
                                                        <div class="score-bar-track">
                                                            <div class="score-bar-fill"
                                                                style="width:<?= min(100, $displayScore) ?>%"></div>
                                                        </div>
                                                    </div>
                                                <?php elseif ($r['status'] === 'rejected'): ?>
                                                    <div style="font-size: 0.75rem; color: var(--danger); font-style:italic;">No
                                                        final score</div>
                                                <?php elseif ($r['status'] === 'needs_revision'): ?>
                                                    <div style="font-size: 0.75rem; color: var(--warning); font-style:italic;">Needs
                                                        Revision</div>
                                                <?php else: ?>
                                                    <div style="font-size: 0.75rem; color: var(--gray); font-style:italic;">Awaiting
                                                        Faculty Validation</div>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                            ...
                                            <td style="font-size:.82rem; color:#5f5f5f; max-width:180px;">
                                                <?= $r['faculty_remarks'] ? htmlspecialchars($r['faculty_remarks']) : '<em>No remarks yet</em>' ?>
                                            </td>
                                            <td style="white-space: nowrap; text-align: center;"
                                                ...
                                                <div
                                                    ...
                                                    <a href="print_fingerprint_report.php?test_id=<?= $r['id'] ?>"
                                                        ...
                                                        style="font-size:0.75rem; padding:4px 8px; text-decoration:none; display:inline-flex; align-items:center;">Print
                                                        Report</a>
                                                </div>
                                            </td>
                                        </tr>
            ...
            <div class="detail-modal-header">
                <h3>
                    <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2.5"
                        stroke-linecap="round" stroke-linejoin="round" style="color:#10b981; margin-right:4px;">
                ...
                <div id="modalContent">
                    <div class="inspect-grid">
                        ...
                        <div>
                            ...
                            <div style="text-align:center; color: #ef4444; font-weight:600; margin-bottom:1rem; display:none;"
                                id="det-img-missing">
                                Image not found.
                            </div>
                        ...
                        <div>
                            ...
                            <div class="coefficient-header" id="det-coefficient-container">
                                ...
                                <div class="overall-score-badge-wrap">
                                    ...
                                    <span class="quality-badge-desc" id="det-quality-badge-desc">Faculty Final
                                        Score</span>
                                </div>
                            </div>
                    ...
                    <div class="analysis-notes-box">
                        ...
                        <div class="info-details-grid">
                            ...
                            <div class="info-detail-row">
                                ...
                                <span class="info-detail-value" id="det-powder"
                                    style="text-transform: capitalize;"></span>
                            </div>
                            <div class="info-detail-row">
                                <span class="info-detail-label">Surface Type:</span>
                                <span class="info-detail-value" id="det-surface"
                                    style="text-transform: capitalize;"></span>
                            </div>
                            <div class="info-detail-row">
                            ...
                            <div class="info-detail-row">
                                ...
                                <span class="info-detail-value" id="det-image-path"
                                    style="font-family: monospace; font-size: 0.75rem; color:#2FBF71; word-break: break-all;"></span>
                            </div>
                            <div class="info-detail-row">
                    ...
                    <div style="text-align: center; margin-top: 1.25rem; font-size: 0.78rem; color: rgba(244, 244, 240, 0.5); font-style: italic;"
                        class="no-print">
                    ...
                    <div style="display:flex; gap:10px; margin-top:1rem;" class="no-print">
                        <button type="button" class="btn btn-secondary" onclick="closeDetailModal()"
                            style="flex:1; background:#163B2A; border-color:rgba(167, 201, 177, 0.25); color:#F4F4F0;">Close</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="detail-modal-header">
                <h3>
                    <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2.5"
                        stroke-linecap="round" stroke-linejoin="round" style="color:#2FBF71;">
            ...
            <div class="detail-modal-body">
                ...
                <div class="compare-img-grid">
                    ...
                    <div class="comp-img-card">
                        ...
                        <div class="comp-img-box" id="comp-orig-box">
                            <img src="" alt="Original Fingerprint" id="comp-orig-img"
                                onerror="showCompPlaceholder('orig')">
                            <div class="comp-placeholder" id="comp-orig-missing" style="display:none;">
                                ...
                                <span class="placeholder-sub">The raw fingerprint file is not accessible for this
                                    record.</span>
                            </div>
                        </div>
                        <div style="font-size: 0.76rem; color: #2FBF71; text-align: center; word-break: break-all; font-weight: 600;"
                            id="comp-orig-filename">TR-0001.jpg</div>
                    </div>
                    ...
                    <div class="comp-img-card">
                        ...
                        <div class="comp-img-box" id="comp-enh-box">
                            <img src="" alt="Enhanced Fingerprint" id="comp-enh-img"
                                onerror="showCompPlaceholder('enh')">
                            <div class="comp-placeholder" id="comp-enh-missing" style="display:none;">
                                ...
                                <span class="placeholder-sub">Preprocessed visualization artifacts have not been
                                    generated for this trial.</span>
                            </div>
                        </div>
                        <div style="font-size: 0.76rem; color: #2FBF71; text-align: center; word-break: break-all; font-weight: 600;"
                            id="comp-enh-filename">TR-0001_enhanced.jpg</div>
                    </div>
                </div>
                ...
                <div class="comp-explanation-box">
                    ...
                    <div class="comp-explanation-text">
                        Original Uploaded Fingerprint represents the raw laboratory submission. Enhanced Fingerprint
                        displays the preprocessing applied before AI evaluation, including grayscale conversion, CLAHE
                        contrast enhancement, and Gaussian blur noise reduction. This visualization is intended for
                        educational purposes only and does not modify the faculty-approved evaluation results.
                    </div>
                </div>
                ...
                <div class="inspect-grid">
                    <div>
                        <div class="column-title" style="color:#FFFFFF;">
                            <svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor"
                                stroke-width="2.5" style="color:#2FBF71;">
                                <line x1="18" y1="20" x2="18" y2="10" />
                                <line x1="12" y1="20" x2="12" y2="4" />
                                <line x1="6" y1="20" x2="6" y2="14" />
                            </svg>
                        ...
                        <div class="coefficient-header">
                            ...
                            <div class="overall-score-badge-wrap">
                                ...
                                <span class="quality-badge-desc" id="comp-quality-badge-desc"
                                    style="color:#D1D5DB;">Faculty Final Score</span>
                            </div>
                        </div>
                        <!-- Metric Bars -->
                        <div id="comp-metrics-container">
                            <div class="metric-item">
                                <div class="metric-info"><span style="color:#D1D5DB;">Ridge Clarity</span><span
                                        id="comp-val-clarity">—</span></div>
                                <div class="metric-bar-track">
                                    <div class="metric-bar-fill" id="comp-fill-clarity"></div>
                                </div>
                            </div>
                            <div class="metric-item">
                                <div class="metric-info"><span style="color:#D1D5DB;">Contrast Quality</span><span
                                        id="comp-val-contrast">—</span></div>
                                <div class="metric-bar-track">
                                    <div class="metric-bar-fill" id="comp-fill-contrast"></div>
                                </div>
                            </div>
                            <div class="metric-item">
                                <div class="metric-info"><span style="color:#D1D5DB;">Minutiae Visibility</span><span
                                        id="comp-val-visibility">—</span></div>
                                <div class="metric-bar-track">
                                    <div class="metric-bar-fill" id="comp-fill-visibility"></div>
                                </div>
                            </div>
                            <div class="metric-item">
                                <div class="metric-info"><span style="color:#D1D5DB;">Fingerprint Sharpness</span><span
                                        id="comp-val-sharpness">—</span></div>
                                <div class="metric-bar-track">
                                    <div class="metric-bar-fill" id="comp-fill-sharpness"></div>
                                </div>
                            </div>
                            <div class="metric-item">
                                <div class="metric-info"><span style="color:#D1D5DB;">Adhesion Quality</span><span
                                        id="comp-val-adhesion">—</span></div>
                                <div class="metric-bar-track">
                                    <div class="metric-bar-fill" id="comp-fill-adhesion"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="info-details-card">
                            <div class="info-detail-row"><span class="info-detail-label">Trial ID:</span><span
                                    class="info-detail-value" id="comp-trial-id" style="color:#2FBF71;"></span></div>
                            <div class="info-detail-row"><span class="info-detail-label">Powder Type:</span><span
                                    class="info-detail-value" id="comp-powder"
                                    style="text-transform:capitalize;"></span></div>
                            <div class="info-detail-row"><span class="info-detail-label">Surface Type:</span><span
                                    class="info-detail-value" id="comp-surface"
                                    style="text-transform:capitalize;"></span></div>
                            <div class="info-detail-row"><span class="info-detail-label">Image Label:</span><span
                                    class="info-detail-value" id="comp-label"></span></div>
                            <div class="info-detail-row"><span class="info-detail-label">Status:</span><span
                                    class="info-detail-value" id="comp-status"></span></div>
                            <div class="info-detail-row"><span class="info-detail-label">AI Score:</span><span
                                    class="info-detail-value" id="comp-ai-score"></span></div>
                            <div class="info-detail-row" id="comp-faculty-row"><span class="info-detail-label">Faculty
                                    Final Score:</span><span class="info-detail-value" id="comp-faculty-score"
                                    style="color:#2FBF71;"></span></div>
                            <div class="info-detail-row"><span class="info-detail-label">Faculty Reviewer:</span><span
                                    class="info-detail-value" id="comp-reviewer"></span></div>
                            <div class="info-detail-row"><span class="info-detail-label">Evaluation Date:</span><span
                                    class="info-detail-value" id="comp-evaluation-date"></span></div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function openDetailModalByData(id) {
            try {
                ...
                if (row) {
                    ...
                    alert('Unable to load record details. Please refresh the page.');
                }
            } catch (err) {
                console.error('Error opening detail modal:', err);
                alert('An error occurred while opening record details.');
            }
        }
        ...
        function openComparisonModalByData(id) {
            try {
                ...
                if (row) {
                    ...
                    console.warn('Record not found for ID:', id);
                    alert('Unable to load comparison record. Please refresh the page.');
                }
            } catch (err) {
                console.error('Error opening comparison modal:', err);
                alert('An error occurred while opening the side-by-side comparison.');
            }
        }
        ...
        function openComparisonModal(row) {
            try {
                if (!row) {
                    alert('Unable to load comparison: Record data is missing.');
                    return;
                }
                ...
                const chipEl = document.getElementById('comp-student-chip');
                if (chipEl) chipEl.textContent = username;

                const trialIdEl = document.getElementById('comp-trial-id');
                if (trialIdEl) trialIdEl.textContent = row.trial_id || 'TR-' + String(row.id).padStart(4, '0');

                const pwdEl = document.getElementById('comp-powder');
                if (pwdEl) pwdEl.textContent = row.powder_type || '';

                const surfEl = document.getElementById('comp-surface');
                ...
                if (row.image_path && row.image_exists) {
                    if (origImg) { origImg.style.display = 'block'; origImg.src = '../view_fingerprint.php?test_id=' + row.id + '&mode=original'; }
                    if (origMissing) origMissing.style.display = 'none';
                    if (origFilename) origFilename.textContent = row.image_path.split('/').pop();
                } else {
                    if (origImg) origImg.style.display = 'none';
                    if (origMissing) origMissing.style.display = 'flex';
                    if (origFilename) origFilename.textContent = 'No original image file';
                }
                ...
                if (row.enhanced_image_path && row.enhanced_image_exists) {
                    if (enhImg) { enhImg.style.display = 'block'; enhImg.src = '../view_fingerprint.php?test_id=' + row.id + '&mode=enhanced'; }
                    if (enhMissing) enhMissing.style.display = 'none';
                    if (enhFilename) enhFilename.textContent = row.enhanced_image_path.split('/').pop();
                } else {
                    if (enhImg) enhImg.style.display = 'none';
                    if (enhMissing) enhMissing.style.display = 'flex';
                    if (enhFilename) enhFilename.textContent = 'No enhanced image file available';
                }
                ...
                // Scores & Metrics
                const aiAccuracy = row.ai_accuracy_score !== null && row.ai_accuracy_score !== undefined ? parseFloat(row.ai_accuracy_score) : (row.accuracy_score !== null && row.accuracy_score !== undefined ? parseFloat(row.accuracy_score) : 0);
                const aiClarity = row.ridge_clarity_score !== null && row.ridge_clarity_score !== undefined ? parseFloat(row.ridge_clarity_score) : 0;
                const aiVisibility = row.visibility_score !== null && row.visibility_score !== undefined ? parseFloat(row.visibility_score) : 0;
                const aiAdhesion = row.adhesion_score !== null && row.adhesion_score !== undefined ? parseFloat(row.adhesion_score) : 0;
                const aiContrast = row.contrast_score !== null && row.contrast_score !== undefined ? parseFloat(row.contrast_score) : 0;

                const hasFacultyScores = row.faculty_final_score !== null && row.faculty_final_score !== undefined;
                const fAccuracy = hasFacultyScores ? parseFloat(row.faculty_final_score) : aiAccuracy;
                const fClarity = hasFacultyScores && row.faculty_ridge_clarity_score !== null && row.faculty_ridge_clarity_score !== undefined ? parseFloat(row.faculty_ridge_clarity_score) : aiClarity;
                const fVisibility = hasFacultyScores && row.faculty_visibility_score !== null && row.faculty_visibility_score !== undefined ? parseFloat(row.faculty_visibility_score) : aiVisibility;
                const fAdhesion = hasFacultyScores && row.faculty_adhesion_score !== null && row.faculty_adhesion_score !== undefined ? parseFloat(row.faculty_adhesion_score) : aiAdhesion;
                const fContrast = hasFacultyScores && row.faculty_contrast_score !== null && row.faculty_contrast_score !== undefined ? parseFloat(row.faculty_contrast_score) : aiContrast;
                ...
                const metricsContainer = document.getElementById('comp-metrics-container');

                if (row.status === 'approved') {
                    if (overallScoreHuge) { overallScoreHuge.style.display = 'block'; overallScoreHuge.textContent = Math.round(fAccuracy) + '%'; }
                    ...
                    const cClar = document.getElementById('comp-val-clarity'); if (cClar) cClar.textContent = fClarity > 0 ? fClarity.toFixed(1) + '%' : '—';
                    const cCont = document.getElementById('comp-val-contrast'); if (cCont) cCont.textContent = fContrast > 0 ? fContrast.toFixed(1) + '%' : '—';
                    const cVis = document.getElementById('comp-val-visibility'); if (cVis) cVis.textContent = fVisibility > 0 ? fVisibility.toFixed(1) + '%' : '—';
                    const cSharp = document.getElementById('comp-val-sharpness'); if (cSharp) cSharp.textContent = fClarity > 0 ? fClarity.toFixed(1) + '%' : '—';
                    const cAdh = document.getElementById('comp-val-adhesion'); if (cAdh) cAdh.textContent = fAdhesion > 0 ? fAdhesion.toFixed(1) + '%' : '—';

                    const fClar = document.getElementById('comp-fill-clarity'); if (fClar) fClar.style.width = fClarity + '%';
                    const fCont = document.getElementById('comp-fill-contrast'); if (fCont) fCont.style.width = fContrast + '%';
                    const fVis = document.getElementById('comp-fill-visibility'); if (fVis) fVis.style.width = fVisibility + '%';
                    const fSharp = document.getElementById('comp-fill-sharpness'); if (fSharp) fSharp.style.width = fClarity + '%';
                    const fAdh = document.getElementById('comp-fill-adhesion'); if (fAdh) fAdh.style.width = fAdhesion + '%';
                } else {
                    if (overallScoreHuge) { overallScoreHuge.style.display = 'none'; overallScoreHuge.textContent = '—'; }
                ...
                const aiScoreEl = document.getElementById('comp-ai-score');
                if (aiScoreEl) aiScoreEl.textContent = aiAccuracy > 0 ? aiAccuracy.toFixed(1) + '%' : 'Awaiting AI Evaluation';

                const statusEl = document.getElementById('comp-status');
                if (statusEl) statusEl.innerHTML = `<span class="badge ${getBadgeClass(row.status)}">${getStatusLabel(row.status)}</span>`;

                const revEl = document.getElementById('comp-reviewer');
                if (revEl) revEl.textContent = row.faculty_validator || 'Faculty Reviewer';
                ...
                const evalDateEl = document.getElementById('comp-evaluation-date');
                if (evalDateEl) evalDateEl.textContent = evalDate;

                const facScoreEl = document.getElementById('comp-faculty-score');
                if (facScoreEl) {
                    if (row.status === 'approved') {
                        facScoreEl.textContent = fAccuracy.toFixed(1) + '%';
                    } else {
                        facScoreEl.textContent = 'Awaiting Faculty Validation';
                    }
                }

                const modalOverlay = document.getElementById('comparisonOverlay');
                if (modalOverlay) {
                    modalOverlay.classList.add('open');
                }
            } catch (err) {
                console.error('Error opening comparison modal:', err);
                alert('An error occurred while opening the side-by-side comparison modal.');
            }
        }
    ...
    `;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            rows.forEach(r => {
                ...
                if (id) {
                    r.onclick = () => openDetailModalByData(id);
                }
            });
